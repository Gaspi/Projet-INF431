...
public class Similarities {
	...
	/**
	 * Initialize the resemblance array with fingerPrint arrays of the files
	 * whose paths are stored in 'urls'.
	 */
	private void initialize() {
		this.resemblances = new int[this.urls.length][];

		for (int i = 0; i < urls.length; i++)
			this.resemblances[i] = HyperLogLog.buildFingerPrint(
					Paths.get(urls[i]), this.hashFunc, this.b, k);
	}

	/**
	 * Display the names of the files which share resemblance beyond a given
	 * threshold.
	 * 
	 * @param threshold
	...
	public void similarFiles(double threshold) {
		...
		for (int i = 0; i < n; i++)
			for (int j = i + 1; j < n; j++) {
				double d = calculateResemblance(this.resemblances[i],
						this.resemblances[j], this.b);
				if (d > threshold) {
					System.out.println("Resemblance between file"
							+ System.lineSeparator() + "    " + this.urls[i]);
					System.out.println("and file" + System.lineSeparator()
							+ "    " + this.urls[j]);
					System.out
							.println("is beyond threshold " + threshold + ".");
					System.out
							.println("-----------------------------------------------------------------");
				}
			}
	 ...
	 * @return The resemblance between the two files, as described in Broder &
	 *         all, Syntactic clustering of the web. In 6th International World
	 *         Wide Web Conference (1997).
	 */
	public static double resemblance(Path pathA, Path pathB, hashFunction func,
			int b, int k) {
	 ...
	 */
	public static double resemblance(String pathA, String pathB,
